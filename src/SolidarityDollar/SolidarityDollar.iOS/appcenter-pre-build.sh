...
APP_CONSTANT_FILE=$APPCENTER_SOURCE_DIRECTORY/SolidarityDollar/SolidarityDollar/AppConstants.cs

if [ -e "$APP_CONSTANT_FILE" ]
then
    echo "Updating ApiUrl to $API_URL in AppConstants.cs"
    sed -i '' 's#ApiUrl = "[-A-Za-z0-9:_./]*"#ApiUrl = "'$API_URL'"#' $APP_CONSTANT_FILE
    echo "Updating ApiKeyDolarSolidario to $APP_SECRETS in AppConstants.cs"
    sed -i '' 's#ApiKeyDolarSolidario = "[-A-Za-z0-9:_./]*"#ApiKeyDolarSolidario = "'$APP_SECRETS'"#' $APP_CONSTANT_FILE
    
    echo "Updating AppCenterKeyiOS to $APP_CENTERIOS in AppConstants.cs"
    sed -i '' 's#AppCenterKeyiOS = "[-A-Za-z0-9:_./]*"#AppCenterKeyiOS = "'$APP_CENTERIOS'"#' $APP_CONSTANT_FILE
     echo "Updating AppCenterKeyAndroid to $APP_CENTERANDROID in AppConstants.cs"
